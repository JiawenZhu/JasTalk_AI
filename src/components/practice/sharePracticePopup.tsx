import React, { useState, useEffect } from "react";
import { Button } from "@/components/ui/button";
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";
import { Copy, Share2 } from "lucide-react";
import { toast } from "sonner";
import Modal from "@/components/dashboard/Modal";

interface SharePracticePopupProps {
  open: boolean;
  onClose: () => void;
  practiceSessionId: string;
  questions: Array<{ id: string; text: string; type: string; difficulty: string; category: string }>;
}

function SharePracticePopup({ open, onClose, practiceSessionId, questions }: SharePracticePopupProps) {
  const [copiedLink, setCopiedLink] = useState(false);
  const [copiedEmbed, setCopiedEmbed] = useState(false);
  const [url, setUrl] = useState<string>("Loading...");
  const [embedCode, setEmbedCode] = useState<string>("Loading...");
  const [activeTab, setActiveTab] = useState("copy");

  const [embedWidth, setEmbedWidth] = useState(1350);
  const [embedHeight, setEmbedHeight] = useState(735);

  useEffect(() => {
    const baseUrl = process.env.LIVE_URL || window.location.origin;
    const practiceURL = `${baseUrl}/practice/shared/${practiceSessionId}`;
    
    if (practiceURL) {
      setUrl(practiceURL);
      setEmbedCode(
        `<iframe src="${practiceURL}" width="${embedWidth}" height="${embedHeight}"></iframe>`,
      );
    }
  }, [practiceSessionId, embedWidth, embedHeight]);

  const copyLinkToClipboard = () => {
    navigator.clipboard.writeText(url).then(
      () => {
        setCopiedLink(true);
        toast.success(
          "Practice interview link copied to clipboard!",
          {
            position: "bottom-right",
            duration: 3000,
          },
        );

        setTimeout(() => setCopiedLink(false), 2000);
        setTimeout(() => onClose(), 1000);
      },
      (err) => console.error("Failed to copy", err.message),
    );
  };

  const copyEmbedToClipboard = () => {
    navigator.clipboard.writeText(embedCode).then(
      () => {
        setCopiedEmbed(true);
        toast.success(
          "Practice interview embed code copied to clipboard!",
          {
            position: "bottom-right",
            duration: 3000,
          },
        );

        setTimeout(() => setCopiedEmbed(false), 2000);
        setTimeout(() => onClose(), 1000);
      },
      (err) => console.error("Failed to copy", err.message),
    );
  };

  if (!open) {
    return null;
  }

  return (
    <Modal open={open} closeOnOutsideClick={false} onClose={onClose}>
      <div className="w-[28rem] flex flex-col">
        <div className="flex items-center space-x-2 mb-4">
          <Share2 className="w-5 h-5 text-blue-600" />
          <p className="text-lg font-semibold">Share Practice Interview</p>
        </div>
        
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-2">Practice Session Details:</h4>
          <div className="text-sm text-blue-800">
            <p>• {questions.length} interview questions</p>
            <p>• Voice-based AI interviewer</p>
            <p>• One attempt per email address</p>
          </div>
        </div>

        <div className="h-auto rounded-xl">
          <Tabs
            value={activeTab}
            className="flex flex-col h-full"
            onValueChange={setActiveTab}
          >
            <div className="w-auto">
              <TabsList>
                <TabsTrigger value="copy">URL</TabsTrigger>
                <TabsTrigger value="embed">Embed</TabsTrigger>
              </TabsList>
            </div>
            <div>
              <TabsContent value="copy" className="w-full">
                <div className="mb-4">
                  <input
                    type="text"
                    value={url}
                    className="w-full p-2 border border-gray-300 bg-gray-100 rounded"
                    readOnly
                  />
                </div>
                <Button
                  className="flex items-center bg-blue-600 hover:bg-blue-700"
                  onClick={copyLinkToClipboard}
                >
                  <Copy size={16} className="mr-2" />
                  {copiedLink ? "Copied!" : "Copy URL"}
                </Button>
              </TabsContent>
              <TabsContent value="embed" className="w-full">
                <div className="mb-4">
                  <input
                    type="text"
                    value={embedCode}
                    className="w-full p-2 border border-gray-300 bg-gray-100 rounded"
                    readOnly
                  />
                </div>
                <div className="flex space-x-4 mb-4">
                  <div className="flex flex-col w-1/2">
                    <label htmlFor="width" className="mb-1 text-sm">
                      Width (px)
                    </label>
                    <input
                      id="width"
                      type="number"
                      min="1050"
                      placeholder="Width"
                      value={embedWidth}
                      className="w-full p-2 border border-gray-300 rounded"
                      onChange={(e) => setEmbedWidth(Number(e.target.value))}
                      onBlur={(e) => {
                        const value = Math.max(1050, Number(e.target.value));
                        setEmbedWidth(value);
                      }}
                    />
                  </div>
                  <div className="flex flex-col w-1/2">
                    <label htmlFor="height" className="mb-1 text-sm">
                      Height (px)
                    </label>
                    <input
                      id="height"
                      type="number"
                      min="700"
                      placeholder="Height"
                      value={embedHeight}
                      className="w-full p-2 border border-gray-300 rounded"
                      onChange={(e) => setEmbedHeight(Number(e.target.value))}
                      onBlur={(e) => {
                        const value = Math.max(700, Number(e.target.value));
                        setEmbedHeight(value);
                      }}
                    />
                  </div>
                </div>
                <Button
                  className="flex items-center bg-blue-600 hover:bg-blue-700"
                  onClick={copyEmbedToClipboard}
                >
                  <Copy size={16} className="mr-2" />
                  {copiedEmbed ? "Copied!" : "Copy Embed Code"}
                </Button>
              </TabsContent>
            </div>
          </Tabs>
        </div>
      </div>
    </Modal>
  );
}

export default SharePracticePopup; 
