-- PHASE 3: Final Status Report (FIXED - Using correct status values and type casting)
...
SELECT 
  ...
  CASE 
    WHEN i.status = 'paused' THEN 'MARKED_PAUSED'
...
SELECT 
  ...
  COUNT(CASE WHEN status = 'paused' THEN 1 END) as paused_interviews,
  COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_interviews,
  COUNT(CASE WHEN status NOT IN ('paused', 'completed') THEN 1 END) as other_status_interviews,
  ROUND(
...
FROM (
  SELECT 
    ...
    ROUND(
      (COUNT(CASE WHEN new_data_status = 'COMPLETE' THEN 1 END)::NUMERIC / 
       GREATEST(COUNT(*), 1)::NUMERIC) * 100, 2
    ) as new_integrity,
    
    -- Old interviews (7 days ago) integrity
    ROUND(
      (COUNT(CASE WHEN old_data_status = 'COMPLETE' THEN 1 END)::NUMERIC / 
       GREATEST(COUNT(*), 1)::NUMERIC) * 100, 2
    ) as old_integrity,
