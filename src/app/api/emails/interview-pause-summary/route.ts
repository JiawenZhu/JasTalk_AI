...
function generatePersonalizedResources(questionAnalysis?: Array<any>, geminiAnalysis?: any): string {
  ...
  if (geminiAnalysis?.questionBreakdown) {
    geminiAnalysis.questionBreakdown.forEach((q: any) => {
      if (q.areasForImprovement) {
        q.areasForImprovement.forEach((area: string) => {
          const lowerArea = area.toLowerCase();
          if (lowerArea.includes('communication') || lowerArea.includes('clarity')) identifiedAreas.add('communication');
    ...
    const pauseSummaryEmail = {
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; max-width: 700px; margin: 0 auto; background: #ffffff; color: #333333;">
          ...
          <div style="padding: 0 20px 20px 20px;">
            <h2 style="color: #111827; margin: 0 0 20px 0; font-size: 24px; font-weight: 600; border-left: 4px solid #059669; padding-left: 16px;">
              📝 Complete Conversation Transcript
            </h2>
            
            <div style="background: #f0fdf4; border: 1px solid #10b981; border-radius: 12px; padding: 24px; margin-bottom: 30px;">
              <p style="margin: 0 0 20px 0; color: #065f46; font-size: 16px; line-height: 1.6; font-weight: 500;">
                Review your complete conversation exchange with ${interviewerName}. Each turn shows the question context and your response quality.
              </p>
              
              <div style="display: grid; grid-template-columns: 1fr; gap: 16px;">
                ${conversationTranscript.map((turn, index) => `
                  <div style="background: #ffffff; border-radius: 8px; padding: 20px; border-left: 4px solid ${turn.speaker === 'USER' ? '#10b981' : '#3b82f6'};">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                      <div style="font-weight: 600; color: ${turn.speaker === 'USER' ? '#065f46' : '#1e40af'}; font-size: 16px;">
                        ${turn.speaker === 'USER' ? '👤 You' : '🤖 ' + interviewerName}
                      </div>
                      <div style="color: #6b7280; font-size: 12px;">Turn ${turn.turnNumber}</div>
                    </div>
                    
                    <div style="margin-bottom: 12px;">
                      <div style="color: #374151; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${turn.text}</div>
                    </div>
                    
                    ${turn.questionContext ? `
                    <div style="background: #f3f4f6; border-radius: 6px; padding: 12px; margin-bottom: 12px;">
                      <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 13px;">Question Context:</div>
                      <div style="color: #6b7280; font-size: 13px;">${turn.questionContext}</div>
                    </div>
                    ` : ''}
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px;">
                      <div style="background: #f9fafb; border-radius: 6px; padding: 12px;">
                        <div style="font-weight: 600; color: #059669; margin-bottom: 4px; font-size: 12px;">Clarity:</div>
                        <div style="color: #6b7280; font-size: 12px;">${turn.responseQuality.clarity}</div>
                      </div>
                      <div style="background: #f9fafb; border-radius: 6px; padding: 12px;">
                        <div style="font-weight: 600; color: #059669; margin-bottom: 4px; font-size: 12px;">Completeness:</div>
                        <div style="color: #6b7280; font-size: 12px;">${turn.responseQuality.completeness}</div>
                      </div>
                      <div style="background: #f9fafb; border-radius: 6px; padding: 12px;">
                        <div style="font-weight: 600; color: #059669; margin-bottom: 4px; font-size: 12px;">Technical Depth:</div>
                        <div style="color: #6b7280; font-size: 12px;">${turn.responseQuality.technicalDepth}</div>
                      </div>
                      <div style="background: #f9fafb; border-radius: 6px; padding: 12px;">
                        <div style="font-weight: 600; color: #059669; margin-bottom: 4px; font-size: 12px;">Confidence:</div>
                        <div style="color: #6b7280; font-size: 12px;">${turn.responseQuality.confidence}</div>
                      </div>
                    </div>
                    ` : ''}
                    
                    <div style="color: #9ca3af; font-size: 11px; margin-top: 12px; text-align: right;">
          ...
          <div style="padding: 0 20px 20px 20px;">
            ...
            <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 12px; padding: 24px; margin-bottom: 30px;">
              <p style="margin: 0 0 20px 0; color: #92400e; font-size: 16px; line-height: 1.6; font-weight: 500;">
              ...
              <div style="display: grid; grid-template-columns: 1fr; gap: 20px;">
                ${questionAnalysis.map((question, index) => `
                  <div style="background: #ffffff; border-radius: 8px; padding: 24px; border-left: 4px solid #f59e0b;">
                    ...
                    <div style="margin-bottom: 16px;">
                      <div style="font-weight: 600; color: #374151; margin-bottom: 8px; font-size: 15px;">Question:</div>
                      <div style="color: #6b7280; font-size: 14px; line-height: 1.5; background: #f9fafb; padding: 12px; border-radius: 6px;">${question.questionText}</div>
                    </div>
                    
                    <div style="margin-bottom: 16px;">
                      <div style="font-weight: 600; color: #374151; margin-bottom: 8px; font-size: 15px;">Your Response:</div>
                      <div style="color: #6b7280; font-size: 14px; line-height: 1.5; background: #f9fafb; padding: 12px; border-radius: 6px;">${question.userResponse}</div>
                    </div>
                    
                    <div style="margin-bottom: 16px;">
                      <div style="font-weight: 600; color: #374151; margin-bottom: 8px; font-size: 15px;">AI Feedback:</div>
                      <div style="color: #6b7280; font-size: 14px; line-height: 1.5; background: #f9fafb; padding: 12px; border-radius: 6px;">${question.aiFeedback}</div>
                    </div>
                    
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                      <div>
                        <div style="font-weight: 600; color: #059669; margin-bottom: 8px; font-size: 14px;">✅ Strengths:</div>
                        <ul style="margin: 0; padding-left: 16px; color: #6b7280; font-size: 13px;">
                          ${question.strengths.map(strength => `<li>${strength}</li>`).join('')}
                        </ul>
                      </div>
                      <div>
                        <div style="font-weight: 600; color: #dc2626; margin-bottom: 8px; font-size: 14px;">💡 Areas for Improvement:</div>
                        <ul style="margin: 0; padding-left: 16px; color: #6b7280; font-size: 13px;">
