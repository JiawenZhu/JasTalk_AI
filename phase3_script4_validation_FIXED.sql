-- PHASE 3: Validation Testing (FIXED - Type casting for ROUND function)
...
SELECT 
  ...
  ROUND(
    (COUNT(CASE WHEN data_status = 'COMPLETE' THEN 1 END)::NUMERIC / 
...
SELECT 
  ...
  ROUND(
    (COUNT(CASE WHEN speaker = 'AGENT' THEN 1 END)::NUMERIC / 
     GREATEST(COUNT(CASE WHEN speaker = 'USER' THEN 1 END), 1)::NUMERIC) * 100, 2
