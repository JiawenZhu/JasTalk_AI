  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      return NextResponse.json(
        { error: 'Admin debug endpoint not available in production' },
        { status: 403 }
      );
    }
    ...
    console.log('🔍 Admin debug subscription query for user:', userId);
    ...
    return NextResponse.json({
      success: true,
      message: 'Admin debug subscription query results',
      userId,
      query: {
        status: 'active',
        user_id: userId
    ...
    console.error('Error in admin debug subscription query:', error);
    return NextResponse.json(
